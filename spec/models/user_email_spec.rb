...
RSpec.describe UserEmail, type: :model do
  it 'does not allow one email to be associated with multiple accounts' do
    u1 = User.create!(primary_email: UserEmail.new(email: 'hello@hooli.xyz'), password: 'blah')
    u2 = User.new(primary_email: UserEmail.new(email: 'hello@hooli.xyz'), password: 'blah')
